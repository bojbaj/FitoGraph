...
{
    public class GetRegionStatesQueryHandler : IRequestHandler<GetRegionStatesQuery, ResultWrapper<GetRegionStatesOutput>>
    {
        ...
        public async Task<ResultWrapper<GetRegionStatesOutput>> Handle(GetRegionStatesQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                Enabled = x.Enabled,
                Selected = x.TRegionCities.SelectMany(z => z.TUsers).Any(z => z.FireBaseId == request.firebaseId),
                Text = x.Title,
                Value = x.Id.ToString(),
