...
{
    public class GetAllFoodTypesQueryHandler : IRequestHandler<GetAllFoodTypesQuery, ResultWrapper<GetAllFoodTypesOutput>>
    {
        private readonly IFireBaseTool _fireBaseTool;
        private readonly AppDbContext _dbContext;
        public GetAllFoodTypesQueryHandler(IFireBaseTool fireBaseTool, AppDbContext dbContext)
        {
            _fireBaseTool = fireBaseTool;
            _dbContext = dbContext;
        }

        public async Task<ResultWrapper<GetAllFoodTypesOutput>> Handle(GetAllFoodTypesQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                Enabled = x.Enabled,
                Selected = x.TUserFoodTypes.Any(z => z.TUser.FireBaseId == request.firebaseId),
                Text = x.Title,
                Value = x.Id.ToString(),
            ...
            .ToList();
            result.Status = true;
            result.Result = new GetAllFoodTypesOutput()
            {
                list = list
            };
